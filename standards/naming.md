...
**HA injects the area prefix automatically — device Names stay bare.** When an entity is first created on a device that is already assigned to an area, HA prepends the area slug to the generated `object_id` itself. Device Names should describe the physical object only, without repeating the area — the area token comes from the area assignment, not from text typed into the Name field.

```
Device Name "Night Lamp" (area: Bathroom) + entity "Motion"  →  binary_sensor.bathroom_night_lamp_motion            ✓
Device Name "Bathroom Night Lamp" (area: Bathroom) + entity "Motion"  →  binary_sensor.bathroom_bathroom_night_lamp_motion   ✗ (area doubled — HA already added it)
```

**This only happens at entity-creation time.** The area slug is locked into the entity_id when the entity is first registered. Renaming the device afterward, or moving it to a different area, does not regenerate existing entity_ids — see §4.3 for the fallout when a device is renamed after the fact, and §4.4 for the case where the device had no area assigned yet when its entities were created.
...
Some integrations (appliances, energy monitors, some cloud integrations) finish creating their entities before the device has an area assigned — setup populates the entities immediately, before the user assigns an area in the UI. Because the area slug is only applied at entity-creation time (§4.1), the result has no area prefix, and assigning the device to an area afterward does not retroactively add one:

```
switch.washer_power             ✗ (area assigned after entities were created)
sensor.energy_monitoring_total  ✗
```

Fix it by overriding each entity_id directly in the registry — the same mechanism as §4.3 Case B:

```
switch.washer_power              →  switch.utility_room_washer_power       ✓
sensor.energy_monitoring_total   →  sensor.household_energy_monitor_power  ✓
```

Device Names stay bare regardless ("Washer", "Energy Monitoring") — do not add an area prefix to the Name field to compensate; per §4.1 it will not regenerate the entity_id and only produces a redundant-looking Name.
