...
| Maximum dim level | `select.averys_room_ceiling_fan_ligh_max_level` | `70%` | The driver's light output plateaus well below full scale — above ~70% the fixture is already at maximum, so the top third of the HA brightness slider produces no visible change. Capping here maps the slider's full travel onto the range the eye can actually see. Mirror of the Minimum dim level floor at the other end. `70%` is the tested value on all rooms; the options list is coarse (`60/65/70/75/80…`). (Friendly name reads "Ligh Max Level" — an Inovelli typo.) |
