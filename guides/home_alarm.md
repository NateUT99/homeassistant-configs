...
Fires when `input_boolean.everyone_sleeping` turns on. This automation is the sole owner of night-mode arming — the arm step was removed from `automation.household_everyone_sleeping_toggle` to centralize the gate here.
...
1. Immediately checks `binary_sensor.exterior_door_open` and `cover.garage_door`. **Open windows never block arming** — they are announced as informational only.
2. If all clear: calls `alarm_control_panel.alarm_arm_night`, then gates the goodnight TTS on the master bedroom door: if `binary_sensor.master_bedroom_door_contact` is already closed, waits 15 seconds; if it is open, waits up to 10 minutes for it to close then waits 15 seconds; if the door never closes within 10 minutes, TTS is skipped. Speaks a goodnight report on the master bedroom HomePod via `script.household_tts_announce` — alarm confirmation, any open windows, tomorrow's weather from `weather.apartment` (fetched via `weather.get_forecasts`).
